...
macro_rules! prng_setup{
    ($algo_type:ty, $algo_name:ident, $data:ident, $data_maker:ident)=>{
        ...
        impl Default for $algo_type {
            fn default() -> Self {
                let prng = PRNG{generator:make_default_stream()};
                $algo_name{$data:$data_maker(prng)}
            }
        }
        impl<T:AlgorithmOutput> From<T> for $algo_type{
            fn from(value: T) -> Self {
                let prng = PRNG{generator:make_stream::<T>(value)};
                $algo_name{$data:$data_maker(prng)}
            }
        }
    }
...
pub fn make_default_stream()->SplitMix64{
    SplitMix64{data:0x43d0f2c5f0c7e0a5}
}
...
pub fn make_4_u32(mut prng:PRNG<SplitMix64>) -> [u32;4]{
    [prng.gen_u32(),prng.gen_u32(),prng.gen_u32(),prng.gen_u32()]
}
///makes 1 u64
...
pub fn make_4_u64(mut prng:PRNG<SplitMix64>) -> [u64;4]{
    [prng.gen_u64(),prng.gen_u64(),prng.gen_u64(),prng.gen_u64()]
}
/// makes a u128
